...
- `check`: JavaScript 구문 검사 → 정산 단위 테스트 12개 → 정적 빌드.
...
- Canvas 2D 파도·서퍼·물보라, 에어·스프레이·배럴 피니시.
...
사용자 지시에 따라 템플릿 승인 전에는 POC를 먼저 완성하고, 이후 제공된 가이드대로 PixiBrown·SceneMaker로 전환합니다. 현재의 HTML/CSS·절차적 Canvas 연출은 POC용입니다.
...
런타임 JavaScript 라이브러리 의존성은 없습니다. 폰트는 Google Fonts를 요청하며, 연결이 안 되면 로컬 폰트로 대체됩니다. 정적 호스팅에는 `dist/` 전체를 사용하고, 파일을 직접 더블클릭하는 대신 HTTP 서버에서 실행합니다.
